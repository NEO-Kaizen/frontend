...
// Administrador (bypass — qualquer `isActive`, inclusive terminal) ou o
// Analista com custódia da solicitação (responsável de triagem ou de
// mapeamento). Gestor e demais perfis são somente leitura. A lista de alvos
// válidos é derivada do PortalConfig em `statusChangeTargets`.
export function canChangeStatusRole(
	...
	if (user.role !== 'Analista' || isTerminal) return null;
	const hasCustody =
