<script lang="ts">
	...
	const requester = $derived(history?.requester ?? null);
	...
	async function handleComposerSend(
		...
		if (!onSendMessage) {
			return { ok: false, error: { message: 'O envio de mensagens não está disponível.' } };
		}
...
<section class="conversation" aria-labelledby="conversation-title">
	...
	{#if !isLoading && !loadError && history}
