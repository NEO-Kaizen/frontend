<script lang="ts">
	...
	interface Props {
		...
		ariaLabel?: string;
		placeholder?: string;
		// Valor considerado "sem seleção". Quando definido e o valor atual difere,
		// um "x" à direita limpa o valor individualmente.
		clearValue?: string;
		// Estado visual do modo de edição: destacar campo alterado ou inválido.
		error?: string;
		disabled?: boolean;
		fieldVariant?: 'editing' | 'dirty' | 'invalid';
		onblur?: () => void;
	}
	...
	const clearAriaLabel = $derived(
		`Limpar seleção de ${(label || ariaLabel || '').replace(/:$/, '')}`
	);
...
<div class="filter-select">
	...
	<div
		...
		class:dirty={fieldVariant === 'dirty'}
		class:disabled
		bind:this={container}
	>
		{#if icon}
